...
- **Force mode** (`trigger-force-enrichment?mode=all`): re-queries every field whose value is **unsettled**. Two states count as settled and are skipped even in force mode:
  ...
  **Failed logo (re)fetch never orphans an existing candidate.** When a logo fetch comes up empty but a logo candidate already exists (`TeamEnrichment.logoUid`, or a logo on `Team`), the prior candidate is kept and its `fieldsMeta.logo.status` is healed to `Enriched` — rather than being set to `CannotEnrich`. Writing `CannotEnrich` while `TeamEnrichment.logoUid` still points at a real `Image` produces a status/column mismatch (observed on prod: `status: CannotEnrich` alongside a populated `logoUid`) that misleads the logo-verification job, which keys off the stored `logoUid`. `CannotEnrich` is written only when there is genuinely no logo on record. (See `resolveLogoMetaOnFetchFailure` in `team-enrichment-logo.util.ts`.)
...
A separate, **logo-only** verification pipeline. It runs independently of the enrichment / judge crons and writes each result to the `TeamLogoVerificationResult` Postgres table (an append-only audit log admins or downstream tooling can query to spot wrong-brand logos). On a `verified + high` verdict it also **auto-promotes** the verified logo to `Team.logo` and stamps `fieldsMeta.logo.judgment` — the only mutation it makes, guarded against user-owned logos (`ChangedByUser`), a logo that rotated between selection and save, and a missing `TeamEnrichment` row. Anything below `verified + high` is audit-only.
...
1. **DB-level candidates** — `team-logo-verification` selects teams that have a logo to verify at **`COALESCE("TeamEnrichment"."logoUid", "Team"."logoUid")`** — i.e. the enriched logo candidate is the source of truth, falling back to the team's current logo — joined to a `TeamEnrichment` row, with the joined `Image.url` non-null. **No `priority` / `isFund` gating is applied** (the enrichment logo, not the team tier, decides eligibility), so the verification population is deliberately broader than the enrichment / judge crons. Ordered by `priority ASC` then `updatedAt DESC` and limited to `LOGO_VERIFICATION_BATCH_SIZE` — higher-priority, recently-changed teams surface first so a freshly-uploaded or freshly-enriched logo gets verified on the next tick.

   This is what lets the VLM verify a logo that exists **only** as an enrichment candidate (`TeamEnrichment.logoUid`) on a team whose `Team.logoUid` is still null — and, on `verified + high`, promote it to `Team.logo`. Keying solely off `Team.logoUid` (the prior behavior) meant enrichment-discovered logos for logo-less teams were never verified and never auto-promoted, since the sole automated promotion path only fires from a verification run.
2. **Per-team `shouldVerifyTeam` gate** — for each candidate, the persistence layer looks up the latest `TeamLogoVerificationResult` for the same `(teamUid, provider)` pair and skips if all of the following hold:
   ...
   - its `logoUid` matches the team's current logo (the same `COALESCE(TeamEnrichment.logoUid, Team.logoUid)`, i.e. the logo wasn't replaced),
   - its `model` matches the currently-resolved model name.

   If the logo was swapped, or the VLM model was upgraded (e.g. `gemini-2.5-flash` → a newer Gemini), the team re-verifies. Teams with no logo on either side are skipped at this stage (defensive — the DB filter already excludes them).
...
| `TEAM_ENRICHMENT_JUDGE_CONCURRENCY` | `5`                        | Max concurrent judge pipelines (each a live AI + optional ScrapingDog call) the cron and the bulk trigger endpoints (`trigger-judgment`, `trigger-force-judgment`) will run at once. Prevents the unthrottled fan-out that starves the connection pool to the AI provider at prod volume (900+ teams) — every failure from that lands as `FailedToJudge`, which still counts as "pending", so the bulk trigger looked like a no-op. |
