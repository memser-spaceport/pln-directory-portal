    ...
    <>
      ...
      <div className="flex pt-32 pb-16">
        <div className="mx-auto flex gap-10">
          <div className="flex w-[917px] flex-col gap-[24px] rounded-[12px]">
            <div className=" mt-10 flex flex-col gap-[24px] rounded-[12px] bg-white p-[30px] shadow-md">
              <Header
                project={selectedProject}
                userHasEditRights={userHasEditRights}
                userHasDeleteRights={userHasDeleteRights}
              />
              <Description content={selectedProject.description} />
            </div>
            {selectedProject.projectLinks?.length > 0 && (
              <div className="rounded-[12px] bg-white p-[30px] shadow-md">
                <ContactAndLinks project={selectedProject} />
              </div>
            )}
            {projectFocusAreas && projectFocusAreas?.length > 0 && (
              <div className="flex flex-col gap-2 rounded-[12px] bg-white p-[30px] shadow-md">
                <h3 className="text-[14px] font-semibold leading-[20px] text-[#475569]">Focus Area</h3>
                <FocusAreaDisplay rawData={focusAreaList} selectedItems={projectFocusAreas} />
              </div>
            )}
          ...
          <div className="mt-10 flex w-[291px] flex-col gap-5">
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  ...
  const focusAreaResponse = await getFocusAreas('Project', {});
  const getMembersResponse = await getMembers({
    'projectContributions.projectUid': query.id + '',
    select:
      'uid,name,image,teamMemberRoles.team,teamMemberRoles.mainTeam,teamMemberRoles.role,teamMemberRoles.teamLead',
    pagination: false,
  });
  ...
  if (selectedProjectResponse.status === 200) {
    ...
    projectFocusAreas = (selectedProjectResponse.body as any)?.projectFocusAreas;
