...
export class ForestAdminService {
    async triggerAirtableSync() {
        try {
            ...
            await Promise.all([highTouchSync(allSlugs["team"]), highTouchSync(allSlugs["member"]), highTouchSync(allSlugs["industry"])])
        } catch (e) {
            console.log(e)
        }
    }
...
function highTouchSync(slug) {
    return axios.post('https://api.hightouch.com/api/v1/syncs/trigger',
    { syncSlug: slug, },
    {
        headers: {
