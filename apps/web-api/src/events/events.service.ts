...
export class EventsService {
  ...
  async createEventLocation(event) {
    try {
      ...
      await this.locationService.createPLEventLocation({ location: eventLocationName.toLowerCase(), timezone: event?.timezone, latitude: Number(event?.address_info?.latitude).toFixed(2), longitude: Number(event?.address_info?.longitude).toFixed(2) });
    } catch (error) {
      throw error;
    }
  }
