...
export class PLEventSyncService {
  ...
  private async fetchEventsFromService(params) {
    try {
      const response = await axios.get(`${process.env.EVENT_SERVICE_URL}/events?status=APPROVED`, {
        headers: {
  ...
  private async createOrUpdateEvents(events, eventMap, locationUid) {
    for (const event of events) {
      ...
      if (existingEvent) {
        ...
        const existingEventUpdatedAt = new Date(existingEvent.updatedAt);
        if (updatedAt > existingEventUpdatedAt) {
          ...
          this.logger.log(`Skipping update for event ${event.event_id} (no newer update found).`);
        }
      } else {
        // Create a new event if it does not exist in the database
        await this.prisma.pLEvent.create({ data: this.mapEventData(event, locationUid) });
      }
    }
  }
