...
export class PLEventGuestsService {
  ...
  */
  ...
  async getAllAggregatedData() {
    try {
      return {
        events: await this.eventService.getAggregatedEvents({ where: { isAggregated: true } }),
        locations: await this.eventLocationsService.getAggregatedLocations({ where: { isAggregated: true } })
      };
    } catch (error) {
      throw new InternalServerErrorException(`Error occured while retrieving aggreagted data: ${error.message}`);
    }
  }
}
