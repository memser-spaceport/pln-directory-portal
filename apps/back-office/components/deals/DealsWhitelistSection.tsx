...
import { useRemoveDealWhitelistMember } from '../../hooks/deals/useRemoveDealWhitelistMember';
import { AddDealWhitelistModal } from './AddDealWhitelistModal';
import clsx from 'clsx';
import { toast } from 'react-toastify';
import { DealWhitelistMember } from '../../utils/services/deal';

interface DealsWhitelistSectionProps {
  authToken: string | undefined;
}

export const DealsWhitelistSection: React.FC<DealsWhitelistSectionProps> = ({ authToken }) => {
  const [showAddModal, setShowAddModal] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');
  const [memberToRemove, setMemberToRemove] = useState<DealWhitelistMember | null>(null);

  const { data: whitelist, isLoading } = useDealsWhitelist({ authToken });

  const removeMutation = useRemoveDealWhitelistMember();
  ...
  const handleRemove = async (member: DealWhitelistMember) => {
    if (!authToken) return;
    ...
    <div className="mt-8 rounded-lg border border-gray-200 bg-white shadow-sm">
      <div className="border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">Deals Access Whitelist</h3>
            <p className="mt-1 text-sm text-gray-500">
              Members who can view and redeem deals in the deals catalog.
            </p>
          </div>
          <button
      ...
      <div className="overflow-x-auto">
        {isLoading ? (
          ...
          <div className="px-6 py-8 text-center text-gray-500">
            ...
            <p className="text-sm text-gray-400">Add members to grant them access to deals</p>
          </div>
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {filteredWhitelist.map((item) => (
                <tr key={item.memberUid} className="hover:bg-gray-50">
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{item.member.email}</td>
      ...
      {memberToRemove && (
        <div className="fixed inset-0 z-[1058] overflow-y-auto">
          <div className="flex min-h-screen items-center justify-center px-4 text-center">
            <div className="fixed inset-0 bg-black bg-opacity-30 transition-opacity" onClick={() => setMemberToRemove(null)} />
            ...
            <div className="relative inline-block w-full max-w-md transform overflow-hidden rounded-lg bg-white p-6 text-left shadow-xl transition-all">
              ...
              <p className="text-sm text-gray-500">
                ...
                <strong>{memberToRemove.member.name || memberToRemove.member.email}</strong> from the deals whitelist?
                <span className="mt-2 block">They will no longer be able to view or redeem deals.</span>
              </p>
