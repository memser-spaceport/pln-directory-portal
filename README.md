...
To run **Husky AI** locally, you should configure the following environment variables. For databases, you can use the provided `docker-compose` setup or any hosted service (see [MongoDB Atlas](https://www.mongodb.com/atlas/database), [Redis Cloud](https://redis.com/redis-enterprise-cloud/)).

### AI Provider Configuration
*Husky AI search answers questions with Gemini by default, using function tools that read teams, members, projects, focus areas, asks, IRL events and forum posts straight from the directory database. No vector database is involved.*
| Name | How to Get/Set | Purpose/Description |
|------|----------------|--------------------|
| `GOOGLE_GENERATIVE_AI_API_KEY` | Create an API key in [Google AI Studio](https://aistudio.google.com/app/apikey) and copy the value | API key for Gemini |
| `GEMINI_MODEL` | Choose a supported model (default `gemini-2.5-flash`) | Gemini model used for chat answers, summaries and thread titles |
| `HUSKY_SEARCH_AI_PROVIDER` | Optional: `gemini` (default), `openai` or `anthropic` | Switches Husky search to another provider; the model then comes from `GEMINI_MODEL`, `OPENAI_LLM_MODEL` or `CLAUDE_MODEL` |
| `OPENAI_API_KEY` / `OPENAI_LLM_MODEL` | Only when `HUSKY_SEARCH_AI_PROVIDER=openai` | OpenAI credentials and chat model |
