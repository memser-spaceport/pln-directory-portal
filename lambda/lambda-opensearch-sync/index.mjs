...
async function getMaxTimestamp(pgClient) {
...
async function indexMembers(pgClient, lastCheckpoint) {
...
async function indexTeams(pgClient, lastCheckpoint) {
    const res = await pgClient.query(`
...
async function indexProjects(pgClient, lastCheckpoint) {
    const res = await pgClient.query(`
...
async function indexEvents(pgClient, lastCheckpoint) {
    const res = await pgClient.query(`
...
export const handler = async () => {
    ...
    await withPgClient(async (client) => {
        const memberRowsChanged = await indexMembers(client, lastCheckpoint);
        const teamRowsChanged = await indexTeams(client, lastCheckpoint);
        const projectRowsChanged = await indexProjects(client, lastCheckpoint);
        const eventRowsChanged = await indexEvents(client, lastCheckpoint);
        ...
        if (rowChanged > 0) {
            ...
            const maxTimestamp = await getMaxTimestamp(client);
